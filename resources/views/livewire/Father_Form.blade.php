    ...
    <div class="col-md-12">
        ...
        <div class="form-row">
            <div class="col">
                ...
                {{-- wire:model-> name = "Email" كأني بقلوا --}}
            ...
            <div class="form-group col-md-6">
                ...
                <select class="custom-select my-1 mr-sm-2" wire:model="Nationality_Father_id">
                    ...
                    @foreach ($Nationalities as $National)
                        <option value="{{ $National->id }}">{{ $National->Nationalities }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col">
                ...
                <select class="custom-select my-1 mr-sm-2" wire:model="Blood_Type_Father_id">
                    ...
                    @foreach ($Type_Bloods as $Type_Blood)
                        <option value="{{ $Type_Blood->id }}">{{ $Type_Blood->Type_Blood }}</option>
                    @endforeach
                </select>
        ...
        @if ($updateMode)
            <button class="btn btn-success nextBtn btn-lg pull-right" wire:click="firstStepSubmit_edit"
                type="button">{{ trans('Parent_trans.Next') }}
            </button>
        @else
            {{-- wire:click -> الضغطة تاعت البوتون --}}
            <button class="btn btn-success nextBtn btn-lg pull-right" style="margin-right: 5px" wire:click="firstStepSubmit"
                type="button">{{ trans('Parent_trans.Next') }}
            </button>
        @endif
